...
async def send_event(websocket: WebSocket, event: str, message: str | None = None):
    if message is None:
        payload = {"event": event}
    else:
...
async def synthesize_sentence_to_pcm(sentence_text: str, target_sample_rate: int = PCM_SAMPLE_RATE) -> bytes | None:
