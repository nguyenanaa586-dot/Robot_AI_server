...
# ==============================================================================
# 1. CẤU HÌNH XOAY VÒNG API KEY (GEMINI 3.6 FLASH)
# ==============================================================================
RAW_KEYS = [
    os.getenv("GEMINI_API_KEY_1"),
    os.getenv("GEMINI_API_KEY_2"),
   # os.getenv("GEMINI_API_KEY_3"),
    os.getenv("GEMINI_API_KEY")  # Key mặc định dự phòng
]
...
def ask_gemini_with_key_rotation(wav_bytes: bytes) -> str:
    """Gửi âm thanh lên Gemini 3.6 Flash - Tự chuyển Key nếu hết lượt"""
    global current_key_index

    if not API_KEYS:
        raise Exception("Server chưa cấu hình GEMINI_API_KEY trên Render!")

    total_keys = len(API_KEYS)

    for attempt in range(total_keys):
        active_key = API_KEYS[current_key_index]
        print(f"[GEMINI 3.6] Đang xử lý bằng API Key #{current_key_index + 1}/{total_keys}...")
        ...
        try:
            ...
            if response and response.text:
                ...
                return "Tao nghe chưa rõ, mày nói lại xem nào."

        except Exception as api_err:
            err_str = str(api_err)
            print(f"[API ERROR Key #{current_key_index + 1}]: {err_str}")

            if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str or "quota" in err_str.lower():
                print(f"[ROTATION] Key #{current_key_index + 1} HẾT LƯỢT! Chuyển sang Key tiếp theo...")
                current_key_index = (current_key_index + 1) % total_keys
            else:
                current_key_index = (current_key_index + 1) % total_keys

    return "u là chời, Hết lượt dùng miễn phí hôm nay rồi đại ca Việt ơi, đại ca mai thử lại nhé."
...
async def chat_audio(request: Request):
    try:
        ...
        # 3. Gửi Audio tới Gemini 3.6 Flash (xoay vòng Key)
        reply_text = ask_gemini_with_key_rotation(wav_bytes)

        # Làm sạch văn bản loại bỏ icon / markdown
